...
class KP:
    ...
    def CloseInsertTransaction(self, trans):
        ...
        """
        trans.close()
        self.connections = [x for x in self.connections if x[1] != trans]
    ...
    def send(self, payload, pl_type="python",
        ...
        if ((pl_type != "RDF-XML") and (encoding != "RDF-XML")):
            ...
            try:
                ...
                """read a url """
                filehandle = urllib.request.urlretrieve(payload)
...
class Connector:
    ...
    def _parse_msg(self, msg):
        ...
        parser.parse(StringIO(msg.decode('ascii')))
        return parsed_msg
...
class TCPConnector(Connector):
    ...
    def send(self, msg):
        length = len(msg)
        sent = self.s.send(msg.encode('ascii'))
        # print msg
        while sent < length:
            # If complete msg could not be sent, try to send the remaining part
            sent += self.s.send(msg[sent:].encode('ascii'))
    ...
    def receive(self):
        ...
        if msg_end_index != -1:
            ...
            # print msg
            msg_unicode = ""
            #msg_unicode = str(msg.encode('string_escape'))
            #msg_ascii = msg_unicode.encode('ascii', 'xmlcharrefreplace')
            return self._parse_msg(msg.encode('ascii', 'xmlcharrefreplace'))
        msg = self.msg_buffer
        self.msg_buffer = ""
        ...
        while True:
            ...
            if msg_end_index == -1:  # Received partial msg
                ...
                msg_unicode = ""
                #msg_unicode = str(msg.encode('string_escape'))
                msg_ascii = msg.encode('ascii', 'xmlcharrefreplace')
